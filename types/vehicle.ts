export const ALL_VEHICLES = [
  "Maxair Hummer 150",
  "Air Creation Racer",
  "CGS Hawk",
  "Spectrum Beaver SS",
  "Aviasud AE 206 Mistral",
  "Wainfan FMX-4 Facetmobile",
  "Remos GX",
  "Tecnam P2002JF Sierra",
  "Colomban MC-15 Cri-Cri",
  "Pegasus Quantum",
  "Davis DA-2A",
  "Evektor SportStar",
  "CTRM Eagle 150",
  "Aceair AERIKS A-200",
  "Rutan Quickie Q2",
  "Rutan Model 61 Long-EZ",
  "ICON A5",
  "Transavia PL-12 Airtruk",
  "Dyke JD-2 Delta",
  "Slepcev Storch MK4",
  "Bede BD-5B",
  "LH Aviation LH-10 Ellipse",
  "Standard J",
  "Piper J-3 Cub",
  "De Havilland DH.87 Hornet Moth",
  "Diamond DA20 Eclipse",
  "American Champion 8GCBC Scout",
  "American Champion 8GCBC Floatplane",
  "Cessna 152",
  "Cessna 172",
  "Cessna R182RG",
  "Piper PA-28R Arrow",
  "Grumman AA-5B Tiger",
  "Piper PA-38-112 Tomahawk",
  "Maule M-5",
  "Piper PA-24-180 Comanche",
  "Sukhoi Su-31",
  "Cirrus SR22",
  "De Havilland Canada DHC-2 Beaver",
  "De Havilland Canada DHC-2 Floatplane",
  "Cessna 188",
  "Kawanishi E7K2",
  "RFB Fantrainer",
  "Mooney M20J",
  "Mooney M20TN Acclaim",
  "Lockheed Model 9 Orion",
  "PAC Cresco 08-600",
  "Piper PA-46-M350",
  "Grob G115e Tutor",
  "Quest Kodiak",
  "Air Tractor AT-602",
  "Diamond DART-550",
  "Pilatus PC-6 B2",
  "Cessna 208B Caravan",
  "Antonov An-2",
  "Beechcraft T-6 Texan II",
  "Pilatus PC-12",
  "Pilatus PC-21",
  "Ilyushin Il-2M3 Shturmovik",
  "Edgley EA-7 Optica",
  "IST XL-15 Tagak",
  "Tecnam P2010",
  "Curtiss F9C Sparrowhawk",
  "Maule MX-7",
  "Planet Satellite",
  "KAI KC-100 Naraon",
  "Extra EA-330SC",
  "Cessna 208A Amphibian",
  "Polikarpov I-16 Type 24 Ishak",
  "Pilatus PC-6 Amphibian",
  "North American T-6G Texan",
  "Daher TBM-900",
  "PAC P-750 XSTOL",
  "Embraer EMB 314 Super Tucano",
  "Fokker D.VII",
  "Mooney M-18 Mite",
  "Messerschmitt Bf-108B-1 Taifun",
  "Mitsubishi A6M3 Zero",
  "Fiat G.55 Centauro",
  "Heinkel He 100 D-1",
  "Vought F4U-1D Corsair",
  "Goodyear F2G-2 Super Corsair",
  "Messerschmitt Bf-109 G-6",
  "Supermarine Seafire FR Mk 47",
  "Stipa-Caproni",
  "Fokker Dr.I",
  "General Atomics MQ-1B Predator",
  "General Atomics MQ-9 Reaper",
  "Macchi M.C.72",
  "North American P-51H-5 Mustang",
  "Yokosuka R2Y1 Keiun",
  "Kyushu J7W Shinden",
  "Focke-Wulf Ta 152H-1",
  "Spirit of St. Louis",
  "Antonov A-40",
  "Model 100P Racer",
  "Bartini M-RD",
  "Aeronautican",
  "Tecnam P2006T",
  "Cessna 337 Skymaster",
  "De Havilland Australia DHA-3 Drover",
  "Lockheed L-12 Electra Junior",
  "Beechcraft Baron 58",
  "Ford Trimotor",
  "Mitsubishi Mu-2B",
  "Britten-Norman BN-2A Islander",
  "Let L-410 Turbolet",
  "De Havilland DHC-6-300 Twin Otter",
  "PTDI N-219 Nurtanio",
  "PTDI N-219 Amphibian",
  "Antonov An-28",
  "Douglas DC-3 Dakota",
  "Basler BT-67",
  "Ilyushin Il-14",
  "Fairchild C-123 Provider",
  "Fairchild Swearingen Metroliner",
  "Saab 340B",
  "Saab 340CF",
  "Beechcraft Super King Air B200",
  "Fokker F27-100 Friendship",
  "ATR 42-600",
  "Bombardier Dash 8 Q100",
  "Douglas DC-4",
  "Aviation Traders ATL-98 Carvair",
  "Embraer 120 Brasilia",
  "Fairchild C-119 Flying Boxcar",
  "CASA CN-235",
  "Xian MA60",
  "Antonov An-8",
  "ATR 72-600",
  "Douglas DC-6B",
  "Douglas DC-6A",
  "Bombardier Dash 8 Q400",
  "Antonov An-24",
  "Lockheed L-749A Constellation",
  "Antonov An-10",
  "Boeing 377 Stratocruiser",
  "Douglas DC-7C",
  "Douglas DC-7F",
  "EM-11 Orka",
  "De Havilland DHC-84 Dragon Rapide",
  "Antonov An-14",
  "Grumman Goose",
  "Beechcraft Starship",
  "De Havilland DHC-6-300 Twin Otter Amph.",
  "Britten-Norman BN-2A Mk III Trislander",
  "Mitsubishi Mu-2J",
  "Short SC.7 Skyvan",
  "Grumman Albatross",
  "De Havilland DHC-7-100",
  "Savoia-Marchetti S.66",
  "CANT Z.511",
  "Ilyushin Il-112",
  "Nord Noratlas",
  "Ilyushin Il-114-100",
  "Ilyushin Il-114T",
  "Saab 2000",
  "Saab 2000 Erieye AEW&C",
  "NAMC YS-11",
  "Fokker 50",
  "Fokker 50F",
  "Lockheed P-3C Orion",
  "CASA C-295",
  "Airbus CC-295 FWSAR Kingfisher",
  "Antonov An-140",
  "Lockheed C-130H Hercules",
  "Antonov An-12",
  "Lockheed L-1049 Super Constellation",
  "Lockheed L-188C Electra",
  "Lockheed L-188CF Electra",
  "Ilyushin Il-18",
  "Ilyushin Il-18GrM",
  "Bristol Britannia 300",
  "Canadair CL-44D4 Swingtail",
  "Conroy CL-44-0 Skymonster",
  "Canadair CP-107 Argus",
  "Fokker 50MPA Enforcer Mk.2",
  "Ilyushin Il-38N",
  "Antonov 181",
  "Aero Ae-45",
  "Rutan Model 202 Boomerang",
  "Tupolev Tu-2 Bat",
  "Piaggio P.180 Avanti",
  "Canadair CL-215 Scooper",
  "Mitsubishi G4M3 Betty",
  "Lockheed P-38 Lightning",
  "Kawanishi H6K4-L",
  "Martin M-130",
  "Tupolev Tu-70",
  "Alenia C-27J Spartan",
  "Latécoère 631",
  "Lockheed WP-3D Orion",
  "Douglas UC-67 Dragon",
  "Lockheed C-121 Super Constellation",
  "Lockheed EC-121 Warning Star",
  "Martin JRM Mars",
  "Breguet 763",
  "Northrop Grumman E-2D Hawkeye",
  "Airbus A400M Atlas",
  "Tupolev Tu-114",
  "Dassault M.D.320 Hirondelle",
  "Grumman XF5F Skyrocket",
  "Focke-Wulf Fw 200 Condor",
  "Avro Lancaster",
  "Consolidated PBY Catalina",
  "Boeing B-17G Flying Fortress",
  "Northrop P-61A Black Widow",
  "Breguet 941",
  "IPTN N-250",
  "Dornier Do X",
  "Kawanishi H8K2-L",
  "Lockheed L-188E Electron",
  "Bristol Brabazon",
  "ShinMaywa US-2",
  "Northrop YB-35",
  "Antonov An-70",
  "Antonov An-22",
  "Tupolev ANT-20",
  "Kalinin K-7",
  "Boeing B-29A Superfortress",
  "Hughes XF-11",
  "Boeing 314 Clipper",
  "Republic XF-12 Rainbow",
  "AVIC AG600M Kunlong",
  "Convair B-36H Peacemaker",
  "Saunders-Roe SR.45 Princess",
  "Tupolev Tu-95MS Bear-H",
  "Convair XC-99",
  "Vought V-173",
  "Wright Flyer",
  "Bo-zone Plane",
  "De Havilland Sea Hornet F.20",
  "Dornier Seastar",
  "Tupolev ANT-28",
  "Hughes H-4 Hercules",
  "Eclipse 500",
  "Honda HA-420 HondaJet",
  "Embraer Phenom 100",
  "Learjet 23",
  "Dassault Falcon 20",
  "Dassault Falcon 20F",
  "Dassault Falcon 20GT",
  "Dassault Falcon 20GTF",
  "Learjet 35",
  "Learjet 35F",
  "Pilatus PC-24",
  "Hawker 400",
  "Embraer ERJ-140",
  "Bombardier CRJ-200",
  "Bombardier CRJ-200SF",
  "Yakovlev Yak-40",
  "Tupolev Tu-134A",
  "BAC 1-11-200",
  "Bombardier CRJ-700",
  "Kawasaki T-4",
  "Embraer E175",
  "Antonov An-158",
  "Bae 146-100",
  "Bae 146-100F",
  "Boeing 737-200",
  "Boeing 737-200F",
  "McDonnell Douglas DC-9-10",
  "McDonnell Douglas DC-9-10F",
  "Boeing 737-700",
  "Northrop T-38 Talon",
  "Airbus A319-100",
  "Yakovlev Yak-42D",
  "Boeing 737-300",
  "Boeing 737-300F",
  "Hawker Siddeley Trident 2E",
  "Boeing 737 MAX 7",
  "Airbus A320-200",
  "Boeing 727-200",
  "Boeing 727-200F",
  "Tupolev Tu-204-100",
  "Tupolev Tu-204-100C",
  "Tupolev Tu-214SUS",
  "Tupolev Tu-214R",
  "Tupolev Tu-154M",
  "McDonnell Douglas DC-8-30",
  "Boeing 757-200",
  "Boeing 757-200SF",
  "Boeing 767-200ER",
  "Boeing 767-200SF",
  "Boeing 707-320B",
  "Boeing 707-320C",
  "Lockheed L-1011-500 TriStar",
  "Lockheed Tristar KC.1",
  "Airbus A310-300",
  "Airbus A310-300F",
  "CMC Leopard",
  "Eclipse 400",
  "Diamond D-Jet",
  "Dassault Falcon 50",
  "Dassault Falcon 50 'Suzanna'",
  "Embraer Phenom 300",
  "IAI Westwind 1124A II",
  "Learjet 75",
  "Gulfstream G100",
  "Cessna Citation Sovereign",
  "L-39C Albatros",
  "Embraer ERJ-145XR",
  "Kawasaki C-1",
  "North American T-2C Buckeye",
  "Northrop F-5E Tiger II",
  "Northrop F-20 Tigershark",
  "Antonov An-74",
  "BAe Systems Hawk T1A",
  "BAe Systems Hawk 200",
  "Fokker 70",
  "Dassault/Dornier Alpha Jet A",
  "Douglas F4D-1 Skyray",
  "Comac ARJ21",
  "Embraer E190",
  "Embraer E190F",
  "BAC 1-11-500",
  "Antonov An-178",
  "Airbus A318-100",
  "Bae 146-300",
  "Bae 146-300F",
  "Sukhoi Superjet 100-95B",
  "English Electric Canberra",
  "Beriev Be-10",
  "KAI FA-50 Fighting Eagle",
  "Bombardier CRJ-1000",
  "Fokker 100",
  "Irkut MC-21-200",
  "Boeing 717-200",
  "Airbus A220-300",
  "Comac C919",
  "Boeing 737-800",
  "Boeing 737-800BCF",
  "Hawker Siddeley Trident 3B",
  "Boeing 737-900",
  "Boeing 737 MAX 8",
  "Airbus A319neo",
  "Lockheed S-3A Viking",
  "Boeing 737 MAX 9",
  "Airbus A321-200",
  "Tupolev Tu-204-300",
  "Vickers VC-10",
  "McDonnell Douglas MD-82",
  "McDonnell Douglas MD-82F",
  "Shanghai Y-10",
  "Airbus A320neo",
  "Ilyushin Il-62 Classic",
  "Ilyushin Il-62MGr",
  "Airbus A321neo",
  "Boeing 757-300",
  "Tupolev Tu-16 Badger",
  "McDonnell Douglas DC-8-62CF",
  "Boeing 767-300ER",
  "Boeing 767-300F",
  "Ilyushin Il-76",
  "Beriev A-50",
  "Lockheed L-1011-1 TriStar",
  "Lockheed L-1011-200F TriStar",
  "McDonnell Douglas DC-8-63",
  "Airbus A300-600R",
  "Airbus A300-600F",
  "McDonnell Douglas DC-10-30",
  "McDonnell Douglas DC-10-30F",
  "Airbus A330-200",
  "Airbus A330-300",
  "Airbus A330-300P2F",
  "Airbus A330-200F",
  "Airbus A330-800neo",
  "Airbus A340-300",
  "Boeing 747-100",
  "Boeing 747-100SF",
  "Boeing 747-100SUD",
  "Boeing 777-200",
  "PZL M-15 Belphegor",
  "Bede BD-5J",
  "Comp Air Jet",
  "Spectrum S-33 Independence",
  "Lockheed Jetstar",
  "Cessna Citation II",
  "Embraer Praetor 600",
  "Bombardier Challenger 350",
  "Learjet U-36A",
  "Avro Canada C102 Jetliner",
  "Myasishchev M-55 Mystic",
  "Tupolev Tu-124",
  "Saab SK60B",
  "Fiat G.91 Gina",
  "Bombardier Challenger 650",
  "Tupolev Tu-104B",
  "De Havilland Venom",
  "Dassault Mercure 100",
  "LTV A-7D Corsair II",
  "Baade 152",
  "Beriev Be-200 Altair",
  "Mikoyan-Gurevich MiG-21",
  "North American F-86D Sabre",
  "Dassault Mirage F1-C",
  "Hawker Hunter FGA.9",
  "McDonnell F3H Demon",
  "Boeing 737-600",
  "Sud Aviation SE 210 Caravelle",
  "Supermarine Scimitar F.1",
  "Grumman F11F-1 Tiger",
  "Convair F-102A Delta Dagger",
  "Convair F-106 Delta Dart",
  "Embraer C-390",
  "Beriev A-40 Albatross",
  "Embraer E195-E2",
  "North American F-100D Super Sabre",
  "Dassault Rafale",
  "Saab JA 37 Viggen",
  "Saab SF 37 Viggen",
  "Sukhoi Su-9U Maiden",
  "Sukhoi Su-9 Fishpot",
  "McDonnell Douglas F/A-18C Hornet",
  "Martin WB-57 Canberra",
  "Vought F8U-1 Crusader",
  "Saab JAS-39E Gripen NG",
  "Lockheed F-104C Starfighter",
  "Convair 880",
  "Convair 880F",
  "General Dynamics F-16C Fighting Falcon",
  "Mitsubishi F-2A Viper Zero",
  "General Dynamics F-16XL Delta Falcon",
  "Lockheed C-141 Starlifter",
  "Boeing 737 MAX 10",
  "Saab J 35 Draken",
  "Chengdu J-10C Menglong",
  "McDonnell Douglas MD-90",
  "Sukhoi Su-27SM Flanker-B",
  "Sukhoi Su-47 Berkut",
  "Kawanishi K-200",
  "Irkut MC-21-300",
  "Sukhoi Su-30MKK Flanker-G",
  "Shenyang J-16D Qianlong",
  "Mikoyan-Gurevich MiG-29A Fulcrum",
  "Mikoyan-Gurevich MiG-29KUB Fulcrum-D",
  "Republic F-105D Thunderchief",
  "Panavia Tornado",
  "Boeing 767-400ER",
  "McDonnell Douglas C-17 Globemaster III",
  "Ilyushin Il-86 Camber",
  "Tupolev Tu-22M2",
  "Ilyushin Il-96-300 Camber",
  "McDonnell Douglas MD-11",
  "McDonnell Douglas MD-11F",
  "Boeing 787-9 Dreamliner",
  "Airbus A340-500",
  "Airbus A330-900neo",
  "Boeing 747SP",
  "Boeing 747-200B",
  "Boeing 747-200SF",
  "Boeing 747-200F",
  "Boeing 747-200SUD",
  "Boeing 777-300",
  "Airbus A350-900ULR",
  "Airbus A350F",
  "Boeing 747-300",
  "Boeing 747-300SF",
  "Boeing 777-300ER",
  "Boeing 777-300ERSF",
  "Boeing 747-400",
  "Boeing 747-400F",
  "Boeing 747-400BCF",
  "Boeing 747-400LCF Dreamlifter",
  "De Havilland Comet 4B",
  "NASA AD-1",
  "MAI Foton",
  "Douglas X-3 Stiletto",
  "Fokker 614",
  "Vickers Nene Vicking",
  "Myasishchev RB-17",
  "Soko J-22A Orao",
  "Mikoyan-Gurevich MiG-15",
  "Messerschmitt Me 262",
  "Cessna Citation X",
  "Republic XF-84H Thunderscreech",
  "Mitsubishi Spacejet M90",
  "Arado Ar 234 B",
  "Arado Ar 234 C",
  "Fairchild Republic A-10C Thunderbolt II",
  "Gulfstream G650ER",
  "Gulfstream G700",
  "English Electric Lightning F.1",
  "McDonnell 220",
  "Lockheed F-117 Nighthawk",
  "Bombardier Global 7500",
  "Scaled Composites Model 281 Proteus",
  "Scaled Composites Model 318 White Knight",
  "Convair 990A Coronado",
  "Hawker Siddeley Vulcan",
  "Boeing YC-14",
  "Eurofighter Typhoon",
  "Sukhoi Su-33 Flanker-D",
  "Shenyang J-15D Feisha",
  "Irkut MC-21-410",
  "Boeing F/A-18E Super Hornet",
  "Boeing EA-18G Growler",
  "Martin P6M-2 SeaMaster",
  "Convair B-58A Hustler",
  "Myasishchev M-4",
  "Sukhoi Su-34 Fullback",
  "North American RA-5C Vigilante",
  "Sukhoi Su-24MR Fencer-E",
  "Xian Y-20 Kunpeng",
  "Xian H-6K Zhanshen",
  "Ilyushin Il-96-400 Camber",
  "Ilyushin Il-96-400T Camber",
  "Boeing 787-10 Dreamliner",
  "Boeing 777-200LR",
  "Boeing 777F",
  "Airbus A350-1000",
  "Airbus A340-600",
  "Boeing 777-9",
  "Boeing 747-8I",
  "Boeing 747-8F",
  "Airbus A380-800",
  "Airbus A300-600ST Beluga",
  "Bell X-1",
  "Lockheed Martin F-22A Raptor",
  "Lockheed Martin F-35A Lightning II",
  "Lockheed Martin F-35C Lightning II",
  "Lockheed U-2A",
  "Lockheed U-2S",
  "McDonnell Douglas F-15C Eagle",
  "McDonnell Douglas F-15 ACTIVE",
  "McDonnell Douglas F-15E Strike Eagle",
  "Boeing F-15SE Silent Eagle",
  "Northrop Tacit Blue",
  "Yokosuka R2Y2 Keiun Kai",
  "Fokker F.26 Phantom",
  "Fairchild Dornier 728",
  "Convair F2Y-1 Sea Dart",
  "Bristol Type 200",
  "Convair XF-92",
  "Leduc 022",
  "Myasishchev M-52",
  "Boeing X-32A",
  "Boom XB-1",
  "Handley Page Victor",
  "KAI KF-21 Boramae Block 1",
  "KAI KF-21 Boramae Block 3",
  "Convair XB-46",
  "Myasishchev M-63",
  "Grumman F-14B Tomcat",
  "Bayraktar Kizilelma",
  "FMA SAIA 90",
  "Boeing E-767",
  "Mikoyan Project 1.44 MFI Flatpack",
  "Mikoyan Project 1.42 MFI Foxglove",
  "Northrop Grumman B-2 Spirit",
  "Rockwell B-1B Lancer",
  "Ilyushin Il-80 Maxdome",
  "Boeing B-52H Stratofortress",
  "BAC TSR-2",
  "Boeing 747SP SOFIA",
  "McDonnell Douglas MD-12",
  "Lun-Class Ekranoplan",
  "Sukhoi T-4 Sotka",
  "Airbus A330-743L BelugaXL",
  "Lockheed C-5 Galaxy",
  "Antonov An-124",
  "Chengdu J-20 Weilong",
  "Kyushu J7W2",
  "Sukhoi Su-57 Felon",
  "Airbus A340-200",
  "Boeing 787-8 Dreamliner",
  "Astraeus ADF-340X Relay",
  "Boeing 7E7",
  "Boeing 7E7F",
  "Kanamori K70",
  "Kanamori K100",
  "Kanamori K200",
  "Kanamori K200F",
  "Kanamori K500",
  "Kanamori K600",
  "Kanamori K700",
  "PZL PZL-230F Skorpion",
  "Astraeus ADF-230X Foulke",
  "Sukhoi S-54",
  "Hecate MPF-02S Archangel",
  "Sukhoi T-75 LTS",
  "Sukhoi Su-75 Checkmate",
  "Boeing Bird Of Prey",
  "KAI KF-21 Boramae Block 1 Double",
  "Northrop Grumman RQ-4B Global Hawk",
  "Tupolev Tu-156",
  "Tupolev Tu-160 Blackjack",
  "North American X-15",
  "North American XB-70A",
  "Beriev SD MBR",
  "Alekseyev KM",
  "Antonov An-225 Mriya",
  "Lockheed SR-71 Blackbird",
  "Lockheed YF-12A",
  "North American XF-108 Rapier",
  "Northrop YF-23 Black Widow II",
  "NPO Molniya BTS-002 OK-GLI",
  "Shin Meiwa GS",
  "Sukhoi KR-860",
  "Aerion SBJ",
  "Tupolev Tu-444",
  "Sud Aviation Super-Caravelle",
  "Tupolev Tu-344",
  "Bristol 223 Mark 3",
  "Boom Overture (2016)",
  "Boom Overture (2022)",
  "Tupolev Tu-144D",
  "Aérospatiale/BAC Concorde",
  "Tupolev Tu-144S",
  "Tupolev Tu-244",
  "Ilyushin Il-72",
  "Boeing 2707",
  "Lockheed L-2000",
  "HyperMach SonicStar",
  "Carreidas 160",
  "Carreidas 160F",
  "Robinson R22",
  "NA-40 Bongo",
  "Bell 47G",
  "Bell 206 JetRanger",
  "Bell OH-58D Kiowa Warrior",
  "MBB Bo-105C",
  "Bell 430",
  "Westland Lynx",
  "AgustaWestland AW159 Wildcat",
  "Bell UH-1 Iroquois",
  "Eurocopter EC-155",
  "AgustaWestland AW139",
  "AgustaWestland AW189",
  "Airbus H145",
  "Mil Mi-8T Hip-C",
  "Mil Mi-8TB Hip-E",
  "Sikorsky UH-60 Black Hawk",
  "Sikorsky MH-60S Sea Hawk",
  "Mil Mi-38",
  "MD Helicopters MD-520N",
  "MD Helicopters MD-530G Defender",
  "Aerospatiale AS-332 Super Puma",
  "AgustaWestland CH-149 Cormorant",
  "Sikorsky S-70 Black Hawk",
  "Boeing AH-64E Apache Guardian",
  "Sikorsky SH-60B Sea Hawk",
  "Sikorsky MH-60T Jay Hawk",
  "Hiller YH-32 Hornet",
  "Kawasaki OH-1",
  "Yakovlev Yak-24",
  "Harbin Z-19 Heixuanfeng",
  "MD Helicopters MD902 Explorer",
  "MD Helicopters MD969 Combat Explorer",
  "MD Helicopters MH-90 Enforcer",
  "Sikorsky S-64 Skycrane",
  "AgustaWestland AW101 ASaC Crowsnest",
  "Boeing CH-47 Chinook",
  "Mil Mi-6",
  "Fiat 7002",
  "Northrop Grumman RQ-8A Fire Scout",
  "Northrop Grumman MQ-8B Fire Scout",
  "Sikorsky S-97 Raider",
  "Mil Mi-26",
  "Sikorsky S-69",
  "Boeing–Sikorsky RAH-66 Comanche",
  "Sikorsky-Boeing SB>1 Defiant",
  "Boeing Vertol XCH-62",
  "Kantanka KTK-02",
  "Tom's Totally Legitimate Helicopter",
  "Mil V-12",
  "Skyship 600",
  "SSSR-V6 Osoaviakhim",
  "Zeppelin NT",
  "R38",
  "Hot-air Balloon",
  "Zeppelin L-31",
  "LZ-7 Deutschland",
  "R101",
  "USS Los Angeles ZR-3",
  "High Altitude Surveillance Balloon",
  "USS Akron ZRS-4",
  "LZ-127 Graf Zeppelin",
  "HMA R100",
  "Airforceproud Balloon",
  "LZ-129 Hindenburg",
  "Flying House",
  "Z.O.M.G.",
  "LZ-133 Spirit of Adventure",
  "Bae Harrier GR.5",
  "AgustaWestland AW609",
  "Yakovlev Yak-38 Forger",
  "Bell-Boeing V-22 Osprey",
  "Bell XV-15",
  "Canadair CL-84 Dynavert",
  "Yakovlev Yak-38U Forger-B",
  "Lockheed Martin F-35B Lightning II",
  "Hawker Siddeley HS.141",
  "Yakovlev Yak-141 Freestyle",
  "Boeing X-32B",
  "Sailing Yacht",
  "Djahazi Container Vessel",
  "Motorboat",
  "G-5-class Motor Torpedo Boat",
  "Fishing Trawler",
  "Riverine Monitor",
  "Deter Crewboat 1000",
  "Damen Stan Patrol 2205 FRP",
  "MSTX Watertaxi",
  "Zodiac RHIB",
  "Emelie-class Inland Container Vessel",
  "Neptun-class Inland Bulk Carrier",
  "Pioneer-class LNG Carrier",
  "Sus-class General Cargo Vessel",
  "Type VIIC U-boat",
  "Osa-class Missile Boat",
  "Barracuda-class Oil Tanker",
  "T2 Tanker",
  "Food Boat",
  "Cutty Sark-class Clipper",
  "Damen Stan Patrol 4207",
  "Beluga-class Submarine",
  "Reliance-class Container Ship",
  "Global-class LNG Carrier",
  "Mutsuki-class Destroyer",
  "NG254-class Container Ship",
  "Sturgeon-class Submarine",
  "B-Delta-class Bulk Carrier",
  "ACL G2 Container Ship",
  "Highway-class Vehicle Carrier",
  "Rio-class Tanker",
  "Pedhoulas-class Bulk Carrier",
  "Ellstrom Elam Plus",
  "Top Fuel Boat",
  "Sparviero-class Patrol Boat",
  "Skjold-class Corvette",
  "Preussen-class Sailing Ship",
  "Boeing 929 Jetfoil",
  "Sigma-class Corvette",
  "Los Angeles-class Submarine",
  "Healy-class Icebreaker",
  "Rosenrot-class LNG Carrier",
  "Maersk B-class Container Ship",
  "Admiral Hipper-class Light Aircraft Carrier",
  "Ghasha-class LNG Carrier",
  "Sacramento-class Support Ship",
  "Cape-class Tanker",
  "Leo-class Cruise Ship",
  "Sovereign Maersk-class Container Ship",
  "Bird-class Container Ship",
  "Special Type 3 Launch Ka-Chi",
  "Enterprize-class Frigate",
  "Fitzgerald-class Bulk Carrier",
  "Visby-class Corvette",
  "O'Ceanos-class Superyacht",
  "Tregurtha-class Bulk Carrier",
  "Saunders-Roe SR.N4",
  "Seawolf-class Submarine",
  "Akula-class Submarine",
  "Estonia-class Cruiseferry",
  "America-class Ocean Liner",
  "LK-60Ya-class Nuclear Icebreaker",
  "Q-Max LNG Carrier",
  "Valemax-class Bulk Carrier",
  "Voyager-class Supertanker",
  "Maersk E-class Container Ship",
  "Lockheed Sea Shadow",
  "Motor Yacht A",
  "O'Mega-class Superyacht",
  "Zumwalt-class Destroyer",
  "Borei-class Submarine",
  "Marlin-class Heavy Lift Vessel",
  "Europa-class Cruiseferry",
  "TI-class Supertanker",
  "Batillus-class Supertanker",
  "Evergreen A-class Container Ship",
  "Innovation-class Container Ship",
  "Typhoon-class Submarine",
  "Olympic-class Ocean Liner",
  "Azzam-class Superyacht",
  "Queen Mary-class Ocean Liner",
  "Iowa-class Battleship",
  "Yamato-class Battleship",
  "Seawise-class Supertanker",
  "Nimitz-class Aircraft Carrier",
] as const;

export type Vehicle = (typeof ALL_VEHICLES)[number];

export function isVehicle(str: string): str is Vehicle {
  return (ALL_VEHICLES as readonly string[]).includes(str);
}
